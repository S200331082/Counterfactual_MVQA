...
class RelevancePropagationBatchNorm2d(nn.Module):
    """Layer-wise relevance propagation for 2D BatchNorm.

    Attributes:
        layer: 2D average batchnorm layer.
        eps: A value added to the denominator for numerical stability.

    """
    def __init__(
        self, layer: torch.nn.BatchNorm2d, eps: float = 1.0e-05, top_k: float = 0.0
    ) -> None:
        super().__init__()
        self.layer = layer
        self.eps = eps
...
class RelevancePropagationConv2d(nn.Module):
    ...
    def __init__(
        ...
        if mode == "z_plus":
            ...
            if self.layer.bias is not None:
                ...
                bias = torch.rand(self.layer.in_channels)
                self.layer.bias = torch.nn.Parameter(torch.zeros_like(bias))
    ...
    def forward(self, a: torch.tensor, r: torch.tensor) -> torch.tensor:
        if self.top_k:
            r = relevance_filter(r, top_k_percent=self.top_k)
        if self.layer.bias.size() != self.layer.out_channels:
            tensor = torch.zeros(self.layer.out_channels).cuda()
            self.layer.bias = torch.nn.parameter.Parameter(tensor, True)
