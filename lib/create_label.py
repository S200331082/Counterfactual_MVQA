...
def create_ans2label(occurence, train_qa_pairs, val_qa_pairs, filename="trainval", root='data'):
    ...
    temp = df[df['answer_type']=="OPEN"]
    for i in range(35):
        temp = temp.append({'image_name':'#%d' % i, 'qid':12992+i, 'answer': '#%d' % i, 'answer_type': 'OPEN'},ignore_index=True)

    open_answers = temp['answer'].unique()
...
def compute_target(answers_dset, ans2label, intersection, name, image_id_col="id", root='data', with_type=False):
    ...
    ***answers_dset should be preprocessed***
    ...
    target = []
    x = answers_dset.copy()
    x.loc[:,'answer'] = x.loc[:,'answer'].apply(lambda y: str(y).lower())
    for id,qa_pair in x.iterrows():
        ...
        if with_type:
            ...
            target.append({
                'qid': qa_pair['qid'],
                'image_name': qa_pair[image_id_col],
                'labels': labels,
                'scores': scores,
                'type': a_type
                })
...
if __name__ == '__main__' :
    ...
    dataset = "slake"  # slake or rad

    source_data_path = r'your_path\%s' % dataset
    ...
    train_path = os.path.join(source_data_path, train_file)
    with open(train_path, 'rb') as f:
        d = json.load(f)
    train = pd.DataFrame(d)

    validate_path = os.path.join(source_data_path, test_file)
    ...
    total_ans2label, intersection = create_ans2label(occurence, train_qa_pairs, val_qa_pairs, filename="trainval", root=created_data_path)     # create ans2label and label2ans

    compute_target(train_qa_pairs, total_ans2label, intersection, 'train', img_col, created_data_path) #dump train target to .pkl {question,image_name,labels,scores}
    compute_target(train_qa_pairs, total_ans2label, intersection, 'train', img_col, created_data_path, with_type=True) #dump train target to .pkl {question,image_name,labels,scores}

    compute_target(val_qa_pairs, total_ans2label, intersection, 'test', img_col, created_data_path)   #dump validate target to .pkl {question,image_name,labels,scores}
    compute_target(val_qa_pairs, total_ans2label, intersection, 'test', img_col, created_data_path, with_type=True)   #dump validate target to .pkl {question,image_name,labels,scores}
